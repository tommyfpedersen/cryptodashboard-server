...
import { getNodeStatus, getBlockAndFeePoolRewards, calculateMiningRewards, getCurrencyVolume, getCurrencyReserve, getMarketCapStats } from './components/verus/verus.js';
...
app.get('/', async (req, res) => {
  ...
  if (cacheReady) {
    ...
    if (req.query.varrrmininghash) {
      calculateUserMiningRewards("vARRR", mainRenderData, req.query.varrrmininghash);
    }
    if (req.query.vdexmininghash) {
      calculateUserMiningRewards("vDEX", mainRenderData, req.query.vdexmininghash);
    }
    if (req.query.chipsmininghash) {
      calculateUserMiningRewards("CHIPS", mainRenderData, req.query.chipsmininghash);
    }

    // address lookup
    if (req.query.vrscaddress) {
      await getAddressBalance("VRSC", mainRenderData, req.query.vrscaddress);
    }
    if (req.query.varrraddress) {
      await getAddressBalance("vARRR", mainRenderData, req.query.varrraddress);
    }
    if (req.query.vdexaddress) {
      await getAddressBalance("vDEX", mainRenderData, req.query.vdexaddress);
    }
    if (req.query.chipsaddress) {
      await getAddressBalance("CHIPS", mainRenderData, req.query.chipsaddress);
    }
